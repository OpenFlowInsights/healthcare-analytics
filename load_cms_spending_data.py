...
def main():
    ...
    for dataset in DATASETS:
        ...
        # 1. Create table using INFER_SCHEMA
        create_table_sql = f"""
        ...
        USING TEMPLATE (
            ...
            FROM TABLE(
                INFER_SCHEMA(
                    LOCATION => '{dataset['s3_path']}',
                    FILE_FORMAT => 'CSV_PARSE_HEADER'
                )
            )
        )
        """
        execute_sql(conn, create_table_sql, f"Create table {dataset['table']}")

        # 2. Create stage for S3 path
        create_stage_sql = f"""
        CREATE OR REPLACE STAGE {SNOWFLAKE_DATABASE}.{SNOWFLAKE_SCHEMA}.{dataset['stage']}
        URL = '{dataset['s3_path']}'
        STORAGE_INTEGRATION = S3_INTEGRATION
        """
        execute_sql(conn, create_stage_sql, f"Create stage {dataset['stage']}")
    ...
    for dataset in DATASETS:
        ...
        FROM {SNOWFLAKE_DATABASE}.{SNOWFLAKE_SCHEMA}.{dataset['table']}
        """
        execute_sql(conn, summary_sql, f"Summary for {dataset['table']}")
